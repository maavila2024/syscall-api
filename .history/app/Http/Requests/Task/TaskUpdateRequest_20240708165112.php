...
class TaskUpdateRequest extends FormRequest
{
    protected function prepareForValidation()
    {
        if ($this->has('expected_date')) {
            $this->merge([
                'expected_date' => Carbon::createFromFormat('Y-m-d', $this->expected_date)->format('YY-mm-dd'),
            ]);
        }

        if ($this->has('finish_date')) {
            $this->merge([
                'finish_date' => Carbon::createFromFormat('Y-m-d', $this->finish_date)->format('YY-mm-dd'),
            ]);
        }
    }
    ...
    public function rules(): array
    {
        return [
            ...
            'expected_date' => 'nullable|date_format:YY-mm-dd',
            'finish_date' => 'nullable|date_format:YY-mm-dd',
        ];
    }
}
